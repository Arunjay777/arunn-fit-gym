...
interface Exercise {
  name: string;
  sets: number; // For interval rounds
  reps: string; // duration / interval style
  weight: string; // level / intensity
  rest: number;
  target: string;
  tip: string;
  image: string;
...
const cardioExercises: Exercise[] = [
  { 
    ...
    weight: 'Level 15', 
    rest: 60, 
    target: 'VO2 Max & Conditioning', 
    tip: 'Sprint at 95%+ maximum effort on treadmill or track, then use the 60 sec rest to walk and recover.',
  ...
  { 
    ...
    weight: 'Damper 6', 
    rest: 90, 
    target: 'Full Body Endurance', 
    tip: 'Drive with your legs first, lean back slightly, and then pull with your arms. Keep strokes fluid.',
  ...
  { 
    ...
    weight: 'Max RPM', 
    rest: 75, 
    target: 'Anaerobic Threshold', 
    tip: 'Push with your legs and pull vigorously with your arms. Maintain a stable spine throughout the interval.',
    image: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=600&auto=format&fit=crop&q=80'
  },
  { 
    ...
    tip: 'Snap your hips forward aggressively to drive the kettlebell to eye level. Do not pull with your arms.',
    image: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=600&auto=format&fit=crop&q=80'
  },
  { 
    name: 'Stair Climber Endurance', 
    sets: 1, 
    reps: '15 min', 
    weight: 'Level 10', 
    rest: 0, 
    target: 'Lactic Acid Threshold', 
    tip: 'Maintain an upright posture. Stepping without leaning heavily on the handle rails targets glutes optimally.',
    image: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=600&auto=format&fit=crop&q=80'
  }
];
...
export default function CardioWorkout() {
  ...
  const handleStartWorkout = () => {
    ...
    cardioExercises.forEach((ex, idx) => {
      initialLogs[idx] = {
        notes: '',
        sets: Array.from({ length: ex.sets }).map(() => ({
          weight: ex.weight.includes('Level') ? ex.weight.replace('Level ', '') : ex.weight.includes('lbs') ? ex.weight.replace(' lbs', '') : '10',
          reps: ex.reps.includes('sec') ? ex.reps.replace(' sec', '') : ex.reps.includes('min') ? ex.reps.replace(' min', '') : ex.reps.replace('m', ''),
          completed: false,
          type: 'Normal'
        }))
      };
  ...
  const handleToggleSetCompletion = (setIdx: number) => {
    ...
    if (isNowCompleted && cardioExercises[currentExerciseIdx].rest > 0) {
      const restSec = cardioExercises[currentExerciseIdx].rest;
  ...
  const handleAddSet = () => {
    ...
    const newLogs = { ...exerciseLogs };
    const setList = [...newLogs[currentExerciseIdx].sets];
    const last = setList[setList.length - 1] || { weight: '10', reps: '30' };
    setList.push({
      weight: last.weight,
      reps: last.reps,
      completed: false,
  ...
  const handleCompleteWorkout = () => {
    ...
    try {
      ...
      Object.entries(exerciseLogs).forEach(([exIdx, log]) => {
        const ex = cardioExercises[parseInt(exIdx)];
      ...
      const newSession = {
        ...
        exercises: loggedExercises.length > 0 ? loggedExercises : [
          { name: 'High Intensity Sprints', sets: 6, reps: [30, 30, 30, 30, 30, 30], weight: 15 },
          { name: 'Rowing Machine Intervals', sets: 4, reps: [500, 500, 500, 500], weight: 6 }
        ],
        color: '#EC4899'
      };
  ...
  const totalPlannedSets = cardioExercises.reduce((sum, ex) => sum + ex.sets, 0);
    ...
    <div className="min-h-screen p-4 lg:p-8 select-none relative pb-20">
      ...
      {showCompletionOverlay && (
        <div className="fixed inset-0 bg-[#030303]/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <TacticalCard glow className="max-w-md w-full p-8 border border-emerald-500/30 text-center relative overflow-hidden">
            ...
            <h2 className="font-mono text-xl font-bold text-white mb-2 tracking-tight">CARDIO ACTION COMPLETE!</h2>
            <p className="font-mono text-xs text-emerald-400 mb-6 uppercase tracking-wider">Your intervals have been logged successfully</p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              ...
              <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center col-span-2">
                <div className="font-mono text-[9px] text-white/40 uppercase tracking-wider mb-1">COMPLETED STATUS</div>
                <div className="font-mono text-sm font-bold text-white">
                  {currentTotalCompletedSets} Rounds / {cardioExercises.length} Conditioning Tasks Complete
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <TacticalHeader title="CARDIO WORKOUT" subtitle="SIMPLE EFFICIENCY • AEROBIC CAPACITY & METABOLIC ENDURANCE" />
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <TacticalCard glow className="relative overflow-hidden p-6 lg:p-8">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-mono text-lg font-bold text-white">CARDIO TRAINING ROUTINE</h3>
                  <p className="font-mono text-xs text-cyan-400/60 uppercase tracking-widest">Target: Aerobic Capacity & Mitochondrial Output</p>
                </div>
              </div>
              
              <p className="font-mono text-sm text-white/70 mb-6 leading-relaxed">
                Elevate your heart rate and accelerate fat oxidation. This routine combines explosive anaerobic intervals with stable aerobic stamina workouts to develop ultimate cardiovascular power.
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  ...
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">45 MIN</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  <div className="font-mono text-[8px] text-white/40 tracking-wider">TOTAL TASKS</div>
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">5 SYSTEMS</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  <div className="font-mono text-[8px] text-white/40 tracking-wider">DIFFICULTY</div>
                  <div className="font-mono text-sm font-bold text-white mt-1">HARD</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  <div className="font-mono text-[8px] text-white/40 tracking-wider">FOCUS TYPE</div>
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">INTERVAL CONDITIONING</div>
                </div>
              </div>
              ...
              {cardioExercises.map((ex, i) => (
                <div key={i} className="bg-white/5 border border-white/5 p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center gap-4 hover:border-cyan-500/30 transition-all overflow-hidden relative">
                  ...
                  <div className="text-right sm:border-l sm:border-white/5 sm:pl-4 min-w-[80px]">
                    <div className="font-mono text-xs font-bold text-cyan-400">{ex.sets} Rounds</div>
                    <div className="font-mono text-[10px] text-white/40">{ex.reps}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            <TacticalCard>
              ...
              <ul className="space-y-3">
                {[
                  { title: "Target Heart Rate Zone", desc: "Keep high interval sprints in Zone 4/5 (90% maximum HR) to push aerobic thresholds higher." },
                  { title: "Active Rest Intervals", desc: "Walk slowly or spin with low resistance during rests; never sit idle completely." },
                  { title: "Hydration Status", desc: "Hydrate incrementally with electrolytes to defend cellular contraction efficiency." }
                ].map((item, idx) => (
          ...
          <div className="space-y-3 lg:col-span-1">
            ...
            <TacticalCard className="mt-4 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-white/5 p-2 rounded-lg border border-white/5 text-center col-span-2">
                  <div className="font-mono text-[8px] text-white/40">EST. CALORIES BURNED</div>
                  <div className="font-mono text-sm font-bold text-rose-400">{Math.round(workoutTimer * 0.22)} kcal</div>
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <TacticalCard className="p-6 relative overflow-hidden">
              ...
              <div className="h-64 sm:h-80 w-full rounded-2xl overflow-hidden relative mb-6 border border-white/10">
                <img
                  src={cardioExercises[currentExerciseIdx].image}
                  alt={cardioExercises[currentExerciseIdx].name}
                  referrerPolicy="no-referrer"
                  className="absolute inset-0 w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent" />
                <div className="absolute bottom-4 left-4 right-4">
                  <p className="font-mono text-[10px] text-cyan-400 uppercase tracking-widest leading-none mb-1">CURRENT CARDIO TASK</p>
                  <h3 className="font-mono text-xl sm:text-2xl font-bold text-white uppercase">{cardioExercises[currentExerciseIdx].name}</h3>
                  <p className="font-mono text-xs text-white/60 uppercase">{cardioExercises[currentExerciseIdx].target}</p>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-white/5 rounded-xl border border-white/5 flex gap-3 text-white/70 mb-5 text-sm font-mono leading-relaxed">
                ...
                <div>
                  ...
                  {cardioExercises[currentExerciseIdx].tip}
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-12 gap-2 text-center text-white/40 font-mono text-[10px] uppercase tracking-wider mb-1">
                  ...
                  <div className="col-span-4">INTENSITY LEVEL</div>
                  <div className="col-span-4">DURATION / DISTANCE</div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-between pt-4">
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                {currentExerciseIdx < cardioExercises.length - 1 ? (
                  <button
                    onClick={() => {
                      playBeep(450, 0.05);
                      setCurrentExerciseIdx(prev => prev + 1);
