...
const backExercises: Exercise[] = [
  { 
    ...
    tip: 'Brace your torso, pull the slack out of the barbell, and press the ground away with your legs.',
  ...
  { 
    ...
    weight: 'Bodyweight', 
    rest: 90, 
    target: 'Lats & Back Width', 
    tip: 'Pull through your elbows, driving your chest up toward the bar, and avoid swinging.',
  ...
  { 
    ...
    tip: 'Keep your hips loaded in a 45-degree hinge. Pull the bar to your lower ribcage.',
  ...
  { 
    ...
    tip: 'Squeeze your elbows down and back. Control the stretch at the very top of each rep.',
  ...
  { 
    ...
    tip: 'Reach forward to stretch your shoulder blades fully, then pull back and squeeze.',
    image: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=600&auto=format&fit=crop&q=80'
  },
  { 
    name: 'Face Pulls', 
    ...
    tip: 'Pull towards your forehead, separating the ropes as you squeeze to engage rear shoulders.',
    image: 'https://images.unsplash.com/photo-1598971639058-fab3c3109a00?w=600&auto=format&fit=crop&q=80'
  }
];
...
export default function BackWorkout() {
  ...
  const handleToggleSetCompletion = (setIdx: number) => {
    ...
    if (isNowCompleted) {
      const restSec = backExercises[currentExerciseIdx].rest;
  ...
  const handleAddSet = () => {
    ...
    const newLogs = { ...exerciseLogs };
    const setList = [...newLogs[currentExerciseIdx].sets];
    const last = setList[setList.length - 1] || { weight: '135', reps: '10' };
    setList.push({
      weight: last.weight,
      reps: last.reps,
      completed: false,
  ...
  const handleCompleteWorkout = () => {
    ...
    try {
      ...
      Object.entries(exerciseLogs).forEach(([exIdx, log]) => {
        const ex = backExercises[parseInt(exIdx)];
      ...
      const newSession = {
        ...
        volume: totalWeightLifted || 16200,
        exercises: loggedExercises.length > 0 ? loggedExercises : [
          { name: 'Deadlift', sets: 4, reps: [8, 8, 5, 5], weight: 405 },
          { name: 'Pull-ups', sets: 4, reps: [10, 10, 8, 8], weight: 0 }
        ],
        color: '#00D4FF'
      };
  ...
  const totalPlannedSets = backExercises.reduce((sum, ex) => sum + ex.sets, 0);
    ...
    <div className="min-h-screen p-4 lg:p-8 select-none relative pb-20">
      ...
      {showCompletionOverlay && (
        <div className="fixed inset-0 bg-[#030303]/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <TacticalCard glow className="max-w-md w-full p-8 border border-emerald-500/30 text-center relative overflow-hidden">
            ...
            <p className="font-mono text-xs text-emerald-400 mb-6 uppercase tracking-wider">Your back exercises have been logged successfully</p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              ...
              <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
                ...
                <div className="font-mono text-lg font-bold text-emerald-400">{(currentLiftedVolume || 16200).toLocaleString()} lbs</div>
              </div>
              <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center col-span-2">
                ...
                <div className="font-mono text-sm font-bold text-white">
                  {currentTotalCompletedSets} sets / {backExercises.length} Exercises Complete
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <TacticalHeader title="BACK WORKOUT" subtitle="SIMPLE EFFICIENCY • LATS & BACK DEPTH PROTOCOL" />
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <TacticalCard glow className="relative overflow-hidden p-6 lg:p-8">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-mono text-lg font-bold text-white">BACK TRAINING ROUTINE</h3>
                  <p className="font-mono text-xs text-cyan-400/60 uppercase tracking-widest">Target: Latissimus Dorsi & Spinal Erectors</p>
                </div>
              </div>
              
              <p className="font-mono text-sm text-white/70 mb-6 leading-relaxed">
                Build back width and thick iron rhomboids with heavy deadlifts, pull-ups, and targeted cable details. Essential for physical balance, athletic posture, and stability.
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  ...
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">70 MIN</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                ...
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  ...
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">STRENGTH & WIDTH</div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <TacticalCard>
              ...
              <ul className="space-y-3">
                {[
                  { title: "Neutral Back Spine", desc: "Never round your lumbar spine during barbell pulls or deadlift load transfers." },
                  { title: "Squeeze the Elbows", desc: "Pull primarily by engaging lats first, focusing on pulling elbows back and down." },
                  { title: "Shoulder Hinge Check", desc: "Keep chest proud, shoulders pulled away from ears at peak contraction." },
                  { title: "Control the Return", desc: "Fight the negative stretch path for 2 seconds to maximize back fiber stimulation." }
                ].map((item, idx) => (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <TacticalCard className="p-6 relative overflow-hidden">
              ...
              <div className="h-64 sm:h-80 w-full rounded-2xl overflow-hidden relative mb-6 border border-white/10">
                <img
                  src={backExercises[currentExerciseIdx].image}
                  alt={backExercises[currentExerciseIdx].name}
                  referrerPolicy="no-referrer"
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  ...
                  <h3 className="font-mono text-xl sm:text-2xl font-bold text-white uppercase">{backExercises[currentExerciseIdx].name}</h3>
                  <p className="font-mono text-xs text-white/60 uppercase">{backExercises[currentExerciseIdx].target}</p>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-white/5 rounded-xl border border-white/5 flex gap-3 text-white/70 mb-5 text-sm font-mono leading-relaxed">
                ...
                <div>
                  ...
                  {backExercises[currentExerciseIdx].tip}
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-between pt-4">
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                {currentExerciseIdx < backExercises.length - 1 ? (
                  <button
                    onClick={() => {
                      playBeep(450, 0.05);
                      setCurrentExerciseIdx(prev => prev + 1);
