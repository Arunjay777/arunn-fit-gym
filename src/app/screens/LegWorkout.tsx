...
const legExercises: Exercise[] = [
  { 
    ...
    weight: '315 lbs', 
    rest: 120, 
    target: 'Quads & Glutes Strength', 
    tip: 'Brace your abs, push your hips back, and squat below parallel with a proud chest.',
  ...
  { 
    ...
    tip: 'Keep your back flat. Push your hips as far back as possible to achieve a deep stretch in hamstrings.',
  ...
  { 
    ...
    rest: 75, 
    target: 'Quad Volume', 
    tip: 'Do not lock your knees at the top. Lower the sled under strict control to 90 degrees.',
  ...
  { 
    ...
    weight: '50 lbs', 
    rest: 60, 
    target: 'Unilateral Balance & Quads', 
    tip: 'Step forward through your heel, keeping your front knee aligned with your toes.',
  ...
  { 
    ...
    weight: '90 lbs', 
    rest: 60, 
    target: 'Hamstring Isolation', 
    tip: 'Squeeze hard at the peak of the bend and control the negative release.',
    image: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=600&auto=format&fit=crop&q=80'
  },
  { 
    name: 'Calf Raises', 
    ...
    rest: 45, 
    target: 'Calves', 
    tip: 'Get a full stretch pause at the bottom and a peak squeeze on your tiptoes.',
    image: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=600&auto=format&fit=crop&q=80'
  },
  { 
    name: 'Leg Extensions', 
    ...
    weight: '120 lbs', 
    rest: 60, 
    target: 'Quadriceps Burnout', 
    tip: 'Hold the peak muscle contraction at the top for a distinct second on every rep.',
    image: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=600&auto=format&fit=crop&q=80'
  }
];
...
export default function LegWorkout() {
  ...
  const handleStartWorkout = () => {
    ...
    legExercises.forEach((ex, idx) => {
      initialLogs[idx] = {
        notes: '',
        sets: Array.from({ length: ex.sets }).map(() => ({
          weight: ex.weight.replace(' lbs', ''),
          reps: ex.reps.includes('-') ? ex.reps.split('-')[0] : ex.reps.includes('/') ? ex.reps.split('/')[0] : ex.reps,
          completed: false,
          type: 'Normal'
        }))
      };
  ...
  const handleToggleSetCompletion = (setIdx: number) => {
    ...
    if (isNowCompleted) {
      const restSec = legExercises[currentExerciseIdx].rest;
  ...
  const handleAddSet = () => {
    ...
    const newLogs = { ...exerciseLogs };
    const setList = [...newLogs[currentExerciseIdx].sets];
    const last = setList[setList.length - 1] || { weight: '225', reps: '10' };
    setList.push({
      weight: last.weight,
      reps: last.reps,
      completed: false,
  ...
  const handleCompleteWorkout = () => {
    ...
    try {
      ...
      Object.entries(exerciseLogs).forEach(([exIdx, log]) => {
        const ex = legExercises[parseInt(exIdx)];
      ...
      const newSession = {
        ...
        volume: totalWeightLifted || 21500,
        exercises: loggedExercises.length > 0 ? loggedExercises : [
          { name: 'Barbell Squat', sets: 5, reps: [8, 8, 6, 6, 5], weight: 315 },
          { name: 'Romanian Deadlift', sets: 4, reps: [10, 10, 8, 8], weight: 225 }
        ],
        color: '#00FF88'
      };
  ...
  const totalPlannedSets = legExercises.reduce((sum, ex) => sum + ex.sets, 0);
    ...
    <div className="min-h-screen p-4 lg:p-8 select-none relative pb-20">
      ...
      {showCompletionOverlay && (
        <div className="fixed inset-0 bg-[#030303]/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <TacticalCard glow className="max-w-md w-full p-8 border border-emerald-500/30 text-center relative overflow-hidden">
            ...
            <p className="font-mono text-xs text-emerald-400 mb-6 uppercase tracking-wider">Your leg exercises have been logged successfully</p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              ...
              <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
                ...
                <div className="font-mono text-lg font-bold text-emerald-400">{(currentLiftedVolume || 21500).toLocaleString()} lbs</div>
              </div>
              <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center col-span-2">
                ...
                <div className="font-mono text-sm font-bold text-white">
                  {currentTotalCompletedSets} sets / {legExercises.length} Exercises Complete
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <TacticalHeader title="LEG WORKOUT" subtitle="SIMPLE EFFICIENCY • POWER & HYPERTROPHY PROTOCOL" />
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <TacticalCard glow className="relative overflow-hidden p-6 lg:p-8">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-mono text-lg font-bold text-white">LEG TRAINING ROUTINE</h3>
                  <p className="font-mono text-xs text-cyan-400/60 uppercase tracking-widest">Target: Quadriceps, Hamstrings & Glutes</p>
                </div>
              </div>
              
              <p className="font-mono text-sm text-white/70 mb-6 leading-relaxed">
                A massive lower body hypertrophy and strength routine featuring barbell squats, hamstring Romanian deadlifts, and high volume accessories. Build powerful legs, stability, and athletic force transfer.
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  ...
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">80 MIN</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  <div className="font-mono text-[8px] text-white/40 tracking-wider">TOTAL EXERCISES</div>
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">7 LIFTS</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  <div className="font-mono text-[8px] text-white/40 tracking-wider">DIFFICULTY</div>
                  <div className="font-mono text-sm font-bold text-white mt-1">ADVANCED</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  <div className="font-mono text-[8px] text-white/40 tracking-wider">FOCUS TYPE</div>
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">POWER & SIZE</div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <TacticalCard>
              ...
              <ul className="space-y-3">
                {[
                  { title: "Knee Alignment Check", desc: "Never let your knees cave inward on squats or lunges; drive them model outward." },
                  { title: "Lumbar Spine Brace", desc: "Keep an extremely tight thoracic brace during squats and deadlift hinges." },
                  { title: "Never Lock Extensions", desc: "Never slide or lock knees at full extension under load on leg press machines." },
                  { title: "Control the Descent", desc: "Lower the barbell slowly on squats to stabilize stabilizer muscles safely." }
                ].map((item, idx) => (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <TacticalCard className="p-6 relative overflow-hidden">
              ...
              <div className="h-64 sm:h-80 w-full rounded-2xl overflow-hidden relative mb-6 border border-white/10">
                <img
                  src={legExercises[currentExerciseIdx].image}
                  alt={legExercises[currentExerciseIdx].name}
                  referrerPolicy="no-referrer"
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  ...
                  <h3 className="font-mono text-xl sm:text-2xl font-bold text-white uppercase">{legExercises[currentExerciseIdx].name}</h3>
                  <p className="font-mono text-xs text-white/60 uppercase">{legExercises[currentExerciseIdx].target}</p>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-white/5 rounded-xl border border-white/5 flex gap-3 text-white/70 mb-5 text-sm font-mono leading-relaxed">
                ...
                <div>
                  ...
                  {legExercises[currentExerciseIdx].tip}
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-between pt-4">
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                {currentExerciseIdx < legExercises.length - 1 ? (
                  <button
                    onClick={() => {
                      playBeep(450, 0.05);
                      setCurrentExerciseIdx(prev => prev + 1);
