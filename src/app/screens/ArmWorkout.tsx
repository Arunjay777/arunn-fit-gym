...
const armExercises: Exercise[] = [
  { 
    ...
    weight: '95 lbs', 
    rest: 75, 
    target: 'Bicep Brachii (Peak)', 
    tip: 'Keep your elbows pinned close to your ribs, contract fully at the top, and do not use momentum.',
  ...
  { 
    ...
    tip: 'Lie flat on a bench, lower the EZ bar controlled to your forehead, keeping elbows parallel.',
  ...
  { 
    ...
    weight: '40 lbs', 
    rest: 60, 
    target: 'Brachioradialis & Outer Biceps', 
    tip: 'Hold the dumbbells with a neutral grip (palms facing each other) to build thick forearms.',
  ...
  { 
    ...
    weight: '60 lbs', 
    rest: 60, 
    target: 'Triceps Isolation', 
    tip: 'Extend your arms fully downwards and separate the rope attachments at the bottom of the movement.',
  ...
  { 
    ...
    tip: 'Set bench to a 45-degree angle. Keep your shoulders back and arms straight down to stretch the bicep fully.',
    image: 'https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?w=600&auto=format&fit=crop&q=80'
  }
];

export default function ArmWorkout() {
  ...
  const handleToggleSetCompletion = (setIdx: number) => {
    ...
    if (isNowCompleted) {
      const restSec = armExercises[currentExerciseIdx].rest;
  ...
  const handleAddSet = () => {
    ...
    const newLogs = { ...exerciseLogs };
    const setList = [...newLogs[currentExerciseIdx].sets];
    const last = setList[setList.length - 1] || { weight: '50', reps: '10' };
    setList.push({
      weight: last.weight,
      reps: last.reps,
      completed: false,
  ...
  const handleCompleteWorkout = () => {
    ...
    try {
      ...
      Object.entries(exerciseLogs).forEach(([exIdx, log]) => {
        const ex = armExercises[parseInt(exIdx)];
      ...
      const newSession = {
        ...
        volume: totalWeightLifted || 6800,
        exercises: loggedExercises.length > 0 ? loggedExercises : [
          { name: 'Barbell Bicep Curls', sets: 3, reps: [10, 10, 10], weight: 95 },
          { name: 'Skull Crushers', sets: 3, reps: [10, 10, 10], weight: 75 }
        ],
        color: '#F59E0B'
      };
  ...
  const totalPlannedSets = armExercises.reduce((sum, ex) => sum + ex.sets, 0);
    ...
    <div className="min-h-screen p-4 lg:p-8 select-none relative pb-20">
      ...
      {showCompletionOverlay && (
        <div className="fixed inset-0 bg-[#030303]/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <TacticalCard glow className="max-w-md w-full p-8 border border-emerald-500/30 text-center relative overflow-hidden">
            ...
            <h2 className="font-mono text-xl font-bold text-white mb-2 tracking-tight">ARM PROTOCOL COMPLETE!</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              ...
              <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
                ...
                <div className="font-mono text-lg font-bold text-emerald-400">{(currentLiftedVolume || 6800).toLocaleString()} lbs</div>
              </div>
              <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center col-span-2">
                ...
                <div className="font-mono text-sm font-bold text-white">
                  {currentTotalCompletedSets} sets / {armExercises.length} Exercises Complete
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <TacticalHeader title="ARM WORKOUT" subtitle="SIMPLE EFFICIENCY • BICEP & TRICEP HYPERTROPHY" />
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <TacticalCard glow className="relative overflow-hidden p-6 lg:p-8">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-mono text-lg font-bold text-white">ARM TRAINING ROUTINE</h3>
                  <p className="font-mono text-xs text-cyan-400/60 uppercase tracking-widest">Target: Biceps Brachii & Triceps Brachii</p>
                </div>
              </div>
              
              <p className="font-mono text-sm text-white/70 mb-6 leading-relaxed">
                Maximize pump, density, and arm strength. This workout utilizes an optimal mix of high tension compound pulls and concentrated extensions to trigger arm hypertrophy.
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  ...
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">50 MIN</div>
                </div>
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                ...
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center sm:text-left">
                  ...
                  <div className="font-mono text-sm font-bold text-cyan-400 mt-1">ARM HYPERTROPHY</div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <TacticalCard>
              ...
              <ul className="space-y-3">
                {[
                  { title: "Elbow Position Stability", desc: "Keep elbows fixed against your sides during curls or pushdowns; avoid lifting them forward." },
                  { title: "Full Muscle Extension", desc: "Squeeze your triceps hard at the bottom of cables for clean muscular isolation." },
                  { title: "No Swing Hype", desc: "Ensure your trunk does not sway or bounce to heave the dumbbells upwards." }
                ].map((item, idx) => (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <TacticalCard className="p-6 relative overflow-hidden">
              ...
              <div className="h-64 sm:h-80 w-full rounded-2xl overflow-hidden relative mb-6 border border-white/10">
                <img
                  src={armExercises[currentExerciseIdx].image}
                  alt={armExercises[currentExerciseIdx].name}
                  referrerPolicy="no-referrer"
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  ...
                  <h3 className="font-mono text-xl sm:text-2xl font-bold text-white uppercase">{armExercises[currentExerciseIdx].name}</h3>
                  <p className="font-mono text-xs text-white/60 uppercase">{armExercises[currentExerciseIdx].target}</p>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-white/5 rounded-xl border border-white/5 flex gap-3 text-white/70 mb-5 text-sm font-mono leading-relaxed">
                ...
                <div>
                  ...
                  {armExercises[currentExerciseIdx].tip}
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-between pt-4">
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                {currentExerciseIdx < armExercises.length - 1 ? (
                  <button
                    onClick={() => {
                      playBeep(450, 0.05);
                      setCurrentExerciseIdx(prev => prev + 1);
